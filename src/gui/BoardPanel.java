	...
	implements MouseListener
{
	...
	/**
	 * Emphasizes the best points to play for the player.
	 * @param g the graphic
	 * @param board the game board
	 * @param points A list of all the points good to play for the player
	 */
	public void emphasizesPoints(Graphics g, int[][] board, int[][] points){
		System.out.println("Liste des points valables :");
		String value = "250";
    ...
    public void paint(Graphics g)
	{
        ...
        int[][] goodPoints = new int[][]{{1,2},{3,4}}; //Add method to get best points.<--------------------------------------- (A deplacer ?)
    	emphasizesPoints(g, board, goodPoints);
        for(int i = 0; i < LogicManager.getBoardSize(); ++i) {
...
//				System.out.print(board[i][j] + "\t");
                if (i % LogicManager.getBoardSize()-1 != 0 || j % LogicManager.getBoardSize()-1 != 0) {
                    if (board[i][j] == 0) {
                        ...
                        g.fillOval(Constants.offSet + (Constants.cell - Constants.hole) / 2 + j * Constants.cell, Constants.offSet + (Constants.cell - Constants.hole) / 2 + i * Constants.cell, Constants.hole/2, Constants.hole/2);
                    	} else {
                    		if (board[i][j] == 1) {
